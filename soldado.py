...
def inimi(prota,inimigos,janela,hitboxs,hitboxsp):
    ...
    for x in inimigos:
        ...
        if(x[3]==True and x[4]==0 and x[5]>=.8):
            if((x[0].get_curr_frame()==17) or (x[0].get_curr_frame()==24)):
                x[5]=0
            if((x[0].get_curr_frame()==15) or (x[0].get_curr_frame()==21)):
            ...
            if(x[0].x<=prota[0].x):
                ...
                x[0].set_final_frame(17)
                if(x[0].get_curr_frame() > 16 or x[0].get_curr_frame()<12):
                    x[0].set_curr_frame(12)
            else:
                x[0].set_initial_frame(18)
                x[0].set_final_frame(24)
                if(x[0].get_curr_frame() < 18):
        ...
        if(x[4]==0 and x[3]==False):
            ...
            x[0].set_final_frame(5)
            if((x[0].get_curr_frame() > 5)):
                x[0].set_curr_frame(0)
                
        if(x[4]!=0):
            x[0].set_initial_frame(6)
            x[0].set_final_frame(11)
            if(x[0].get_curr_frame() > 11 or x[0].get_curr_frame() < 6):
                x[0].set_initial_frame(6)
